...
class DownloadBufferTests: XCTestCase {
    ...
    func testDownload_whenAppendingTotalLength_shouldContainsOneFragment() async throws {
        ...
        // Then
        let bytes = try await Array(AsyncBytes(download.stream))
    ...
    func testDownload_whenAppendingErrorBeforeData_shouldBeEmpty() async throws {
        ...
        do {
            for try await data in AsyncBytes(download.stream) {
    ...
    func testDownload_whenAppedingDiferentSizes_shouldMergeByLength() async throws {
        // Given
        let length = 1_024
        ...
        var download = DownloadBuffer(readingMode: .length(length))
