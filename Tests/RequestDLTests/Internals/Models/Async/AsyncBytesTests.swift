...
class AsyncBytesTests: XCTestCase {

    var stream: DataStream<DataBuffer>!
    ...
    func testAsyncBytes_whenAppendData_shouldContainsAll() async throws {
        ...
        let bytes = AsyncBytes(stream)

        // When
        stream.append(.success(DataBuffer(part1)))
        stream.append(.success(DataBuffer(part2)))
        stream.close()
