...
struct DiskStorageTests {
    ...
    func removeAll_whenAnEntryIsStillBeingWritten_doesNotStallWaitingForItToComplete() async throws {
        try await withTemporaryFileURL(createPath: false) { directoryURL in
            ...
            // by-key lookup path is right to retry past that (see the test above); a full-directory
            // scan is not: `removeAll()`/`removeAll(since:)`/`freeSpace` see every entry in the
            // directory, not one write this call is racing, so retrying this one for 15s would
            // stall the whole call for however many writes happen to be in flight.
