...
class HeadersRefererTests: XCTestCase {
    ...
    func testReferer() async throws {
        ...
        XCTAssertEqual(request.headers.getValue(forKey: "Referer"), "https://www.example.com/")
    }

    func testRefererWithHTML() async throws {
        let property = TestProperty(Headers.Referer("https://www.example.com/page1.html"))
        let (_, request) = try await resolve(property)
        XCTAssertEqual(request.headers.getValue(forKey: "Referer"), "https://www.example.com/page1.html")
    }
    ...
    func testRefererWithPathAndQuery() async throws {
        ...
        let (_, request) = try await resolve(property)
        XCTAssertEqual(request.headers.getValue(forKey: "Referer"), "https://www.google.com/search?q=apple")
    }
