...
// Every test here calls `configuration.build().httpClientConfiguration`, and both `Output`
// (the type `build()` returns) and `.httpClientConfiguration`'s `HTTPClient.Configuration`
// (AsyncHTTPClient) only exist under `canImport(NIOCore)`. See the main declaration's own doc
// comment, in `InternalsSessionConfigurationTests.swift`.
...
extension InternalsSessionConfigurationTests {
