...
// Every test here goes through `Internals.PrivateKey.build()`, which only exists under
// `canImport(NIOCore)` (it returns `NIOSSLPrivateKey`).
...
struct InternalsPrivateKeyTests {
    ...
    func private_whenPEMWithPassword_shouldBeValid() async throws {
        // Given
        let password = SecureBytes("password123".utf8)
    ...
    private static let genuinelyEncryptedPEMPassword = SecureBytes("testpassword123".utf8)
