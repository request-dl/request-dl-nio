...
| 5 | `Internals/` | ⛔ indirect only | — | Test via public API, never directly. 23 files below 85%, all in scope by policy exclusion, not investigated. |
...
1. [ ] `coverage-report.txt`/`coverage-lines.txt` at repo root reflect the end-of-batch-4 state (regenerated after all four fixes landed, full 895-test suite green). Re-run `./Scripts/coverage.sh` at the start of the next session only if uncommitted changes have landed since, or just to be safe.
2. [ ] **Priorities 1–4 (`Properties/Sources/`, `Tasks/Sources/Modifiers/`, `Tasks/Sources/Interceptors/`, `Request/` + rest of `Tasks/`) are all done** (see §2/§3 batch-4 entry) — a full-repo sweep confirmed zero files below 85% line coverage in those trees except the 4 already-investigated-and-skipped files from batches 1–2. Don't re-investigate documented skips/left-alone files without a new angle.
3. [ ] **Open decision for the next session: whether to start on priority 5, `Internals/`.** The stated project rule is "test via public API, never directly" — but 23 `Internals/` files currently sit below 85% line coverage (e.g. `Internals.Log.swift` 36.67%, `FilePath+.swift` 42.11%, `Internals.SystemProxyResolver.swift` 44.93%, full list reproducible via the sweep script in the batch-4 entry). Ask the user: (a) leave `Internals/` alone entirely since it's "indirect only" by design, (b) attempt to raise coverage of `Internals/` files strictly through the public `RequestDL` API surface (no `@testable` direct construction of `Internals` types), or (c) some other scope. Do not start writing `Internals/` tests without this decision — the "indirect only" phrasing is ambiguous between "don't test it as a batch priority" and "public-API-only test style if touched."
4. [ ] If greenlit to look at `Internals/`: before writing tests for any file, check `coverage-report.txt` for its current line/region coverage and cross-reference `coverage-lines.txt` for exact uncovered lines — same workflow as batches 1–4. Expect many gaps to be Darwin/Linux platform-branch or fault-injection cases (disk I/O errors, DNS resolution failures) that may turn out untestable without breaking the "public API only" constraint — that's fine, document and skip per the established pattern, don't force it.
...
6. [ ] Apply the `@autoclosure` gotcha broadly, not just for `Internals.TaskLogger`: any `swift-log` `Logger.debug(...)`/`.trace(...)` call with string interpolation needs a logger whose level admits it, or the interpolation closure shows 0 executions despite the enclosing function running fine. For a plain `Logger` value (not the task-local `RequestEnvironmentValues.current.logger`), just set `.logLevel = .trace` directly on the instance — no need for `Logger.withTesting`'s task-local machinery unless the code under test reads the task-local specifically.
7. [ ] For defensive/fallback branches unreachable through real integration flows, check whether `@testable import` direct construction of the internal type (or, per batch 4, a minimal custom protocol conformance like a hand-written `Combine.Subscriber`) can reach it before writing it off — see §5 for the running list of examples.
8. [ ] After the next session's work: `swift test`, `swift format format --in-place --recursive Tests`, re-run `./Scripts/coverage.sh`, diff against this session's numbers, update this file again.
9. [ ] Do not commit/push new tests without going through `/commit-push` and explicit user confirmation.
10. [ ] `coverage-report.txt`, `coverage-lines.txt`, and `job-logs.txt` (if it reappears) are still untracked and not in `.gitignore` as of batch 4 — still unresolved after being flagged at the end of batches 2 and 3 too. Flag to the user again at the next commit; consider just adding them to `.gitignore` if the user agrees rather than re-flagging indefinitely.
