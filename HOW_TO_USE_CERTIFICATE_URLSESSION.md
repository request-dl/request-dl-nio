# Using `Certificate`/`PrivateKey` (mTLS) under the URLSession executor

**Status:** shipped. The TLS/mTLS mapping this document describes is implemented (`URLSESSION_TASK.md` Phase 5e — `Internals.RawBytesIdentityBuilder`, `Internals.URLSessionIdentityPolicy`), and the public API to actually select the URLSession executor for a request has landed too (Phases 6/7): `Session.preferredExecutor(.urlSession)` (a soft hint, falls back safely if the configuration can't run there) or `Session.requiredExecutor(.urlSession)` (a hard pin, throws `ExecutorRequirementError` up front if it can't). On Darwin, `.urlSession` is also `resolveExecutor()`'s own default preference for a compatible configuration, so a request using `Certificate`/`PrivateKey` with no executor modifier at all may already be running over `.urlSession` today. Based on the validated spike for [request-dl-nio#287](https://github.com/request-dl/request-dl-nio/discussions/287). See [URLSESSION_REPORT.md](URLSESSION_REPORT.md) for the full technical background.
...
A request using `Certificate`/`PrivateKey` under `.urlSession` that fails to build a client identity throws `ClientIdentityError` (public, `RequestDL`) — `error.reason` names the specific cause, and both `"\(error)"`/`error.description` and `error.localizedDescription` carry the same actionable text (the latter following Foundation's usual generic wording would otherwise hide it — see `URLSESSION_TASK.md` Phase 8). `.reason == .missingKeychainSharingEntitlement(operation:)` is the case this section covers.
