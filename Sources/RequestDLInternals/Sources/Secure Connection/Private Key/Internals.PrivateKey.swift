...
extension Internals {
    ...
    package struct PrivateKey: Sendable, Equatable {
        ...
        package let password: Internals.SecureBytes?
        ...
        package init(_ file: String, format: Internals.Certificate.Format, password: Internals.SecureBytes) {
            self.source = .file(file)
            self.format = format
            self.password = password
        }

        package init(_ bytes: [UInt8], format: Internals.Certificate.Format, password: Internals.SecureBytes) {
            self.source = .bytes(bytes)
            self.format = format
            self.password = password
        }
