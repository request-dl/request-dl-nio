...
extension Internals {
    ...
    /// begins. That ordering alone isn't enough to make teardown safe, though: Swift zeroes a
    /// `weak var` as soon as the referenced object's strong refcount hits zero, independently of
    /// `lock` -- before the dying `IdentityHandle`'s own `deinit` body (i.e. `release(label:)`)
    /// gets a chance to even ask for it. So a second caller building for the same label can win
    /// the lock first, see the already-zeroed weak reference, and register a brand new handle for
    /// that label before the original handle's `release(label:)` ever runs. `release(label:)`
    /// only deletes the Keychain items (and only then clears the registry slot) when nothing live
    /// is registered for the label at that point, so it can tell "nobody rebuilt while I was
    /// waiting for the lock" apart from "a new handle already owns this label" and never deletes
    /// out from under the latter.
    package final class IdentityManager: @unchecked Sendable {
        ...
        /// `label` right now. A concurrent `handle(for:build:)` call can have already rebuilt and
        /// registered a fresh handle for the same label between this handle's weak reference
        /// zeroing and this call actually acquiring `lock` (see this type's own doc comment); when
        /// that has happened, that newer handle owns these Keychain items now, and this call must
        /// leave both the registry slot and the Keychain alone.
