...
extension Property {
    ...
    /**
     ...
     This can be particularly useful if the server does not accept duplicated header keys. By setting the separator, the header values will be merged into a single value. This functionality is supported across all RequestDL operations.

     The default behavior of SwiftNIO is to send and receive headers value by value. Since this behavior is well-suited for SwiftNIO, RequestDL provides the flexibility to change it to meet the specific requirements of the server specifications.
